...
import { listAdminPages, deletePage, type Page } from '@/api/cmsContent'
...
async function load() {
  loading.value = true
  pages.value = await listAdminPages()
  loading.value = false
}
  ...
  <DashboardLayout>
    <div class="mx-auto max-w-5xl px-4 py-8">
      <div class="mb-6 flex items-center justify-between">
        <h1 class="text-xl font-semibold text-stone-900 dark:text-stone-50">Halaman Statis</h1>
        <AppButton @click="router.push({ name: 'admin-pages-create' })"><AppIcon name="plus" :size="18" /> Tambah</AppButton>
      </div>
        ...
        <table class="w-full text-left text-sm">
          ...
          <tbody class="divide-y divide-stone-100 dark:divide-stone-800">
            <tr v-for="page in pages" :key="page.id">
              ...
              <td class="px-4 py-2">{{ page.translations[0]?.title ?? page.slug }}</td>
              <td class="px-4 py-2">{{ page.slug }}</td>
              <td class="px-4 py-2 capitalize">{{ page.status }}</td>
              <td class="px-4 py-2 text-right">
                <button class="mr-3 text-brand-600 hover:underline dark:text-brand-400" @click="router.push({ name: 'admin-pages-edit', params: { id: page.id } })">Edit</button>
                <button class="text-red-600 hover:underline dark:text-red-400" @click="remove(page)">Hapus</button>
              </td>
            </tr>
          </tbody>
        </table>
