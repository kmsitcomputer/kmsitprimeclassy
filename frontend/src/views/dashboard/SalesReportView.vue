...
import { getSalesReport, downloadReportXlsx, type SalesReportRow } from '@/api/reports'
...
const items = ref<SalesReportRow[]>([])
const loading = ref(true)
const currentPage = ref(1)
const lastPage = ref(1)
const filters = ref({ from: '', to: '', search: '', korsal_id: '', status: '', agent_id: undefined as number | undefined })
...
async function load(page = 1) {
  ...
  const result = await getSalesReport({
    ...
    korsal_id: filters.value.korsal_id ? Number(filters.value.korsal_id) : undefined,
...
<template>
  <DashboardLayout>
    <h1 class="mb-1 font-display text-2xl font-semibold text-stone-800 dark:text-stone-100">Laporan Sales</h1>
    <p class="mb-5 text-sm text-stone-500 dark:text-stone-400">Semua sales di jaringan Anda beserta transaksi dan fee yang didapat.</p>
    ...
    <div v-else class="overflow-x-auto rounded-xl border border-stone-200 dark:border-stone-800">
      <table class="w-full text-left text-sm">
        ...
        <tbody class="divide-y divide-stone-100 dark:divide-stone-800">
          <tr v-for="row in items" :key="row.sales_id">
            <td class="px-4 py-2">{{ row.sales_name }}</td>
            <td class="px-4 py-2">{{ row.sales_phone }}</td>
