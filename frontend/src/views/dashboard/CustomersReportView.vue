...
import { getCustomersReport, downloadReportXlsx, type CustomerReportRow } from '@/api/reports'
...
const items = ref<CustomerReportRow[]>([])
...
<template>
  <DashboardLayout>
    <h1 class="mb-1 font-display text-2xl font-semibold text-stone-800 dark:text-stone-100">Laporan Pelanggan</h1>
    <p class="mb-5 text-sm text-stone-500 dark:text-stone-400">Total konsumen yang melakukan transaksi di jaringan Anda.</p>
    ...
    <div class="mb-5 flex flex-wrap items-end gap-3 rounded-xl border border-stone-200 bg-white p-3 dark:border-stone-800 dark:bg-stone-900">
      ...
      <label class="text-sm text-stone-600 dark:text-stone-300">
        Cari
        <input v-model="filters.search" type="text" placeholder="Nama atau telepon" class="mt-1 block w-48 rounded-lg border border-stone-200 bg-white px-3 py-2 text-sm dark:border-stone-700 dark:bg-stone-950" />
      </label>
      ...
      <table class="w-full text-left text-sm">
        <thead class="bg-stone-50 text-stone-500 dark:bg-stone-900 dark:text-stone-400">
          <tr>
            ...
            <th class="px-4 py-2">Order Terakhir</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-stone-100 dark:divide-stone-800">
          <tr v-for="row in items" :key="row.konsumen_id">
            ...
            <td class="px-4 py-2 text-right">{{ formatRupiah(row.total_spent) }}</td>
            <td class="px-4 py-2">{{ formatDate(row.last_order_at) }}</td>
          </tr>
