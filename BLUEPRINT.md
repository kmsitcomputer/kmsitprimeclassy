...
| Testing | PHPUnit; 45 file test dengan 500+ method test terdeklarasi (517 lulus pada eksekusi suite penuh terakhir), dijalankan terhadap database MySQL test terisolasi |
...
#### 3.4.1. Resi Pengiriman Thermal (Print Resi)

- Setiap **Shipment** (bukan Order) punya resi thermal sendiri, dicetak dari `GET /shipments/{shipment}/receipt` (`ShipmentPolicy::printReceipt`) — mode resi diturunkan otomatis dari state shipment, tidak pernah dipilih pemanggil: `pre_pickup` selama `shipment.shipped_at` masih kosong, `post_pickup` begitu kurir menekan "Ambil" (event yang sama dengan transisi `diproses → dikirim`; proyek ini tidak punya kolom `picked_up_at` terpisah — `shipped_at` itulah timestamp pickup otentik).
- Resi murni operasi **baca** — mencetak/reprint tidak pernah mengubah status order/item/shipment, `shipped_at`, assignment kurir, ongkir, atau payment. Setiap print (termasuk reprint) dicatat ke `activity_logs` (`shipment.receipt_printed`, dengan flag `is_reprint`).
- Karena satu order bisa punya beberapa Shipment/kurir (lihat §3.2/§6 poin 4), resi hanya menampilkan item yang memang berada pada Shipment tersebut — item milik kurir lain pada order yang sama tidak pernah bocor ke resi ini.
- Konten resi: nomor order, tanggal order, penerima, alamat (dari snapshot order — bukan alamat konsumen saat ini), daftar item (SKU/nama dari snapshot), tanggal kirim, kurir + waktu pickup aktual (mode `post_pickup`), indikator pembayaran minimal (COD/DP/LUNAS tanpa nominal fee internal). Tidak ada data finansial internal (agent/sales/courier fee), tidak ada koordinat mentah/data routing API, tidak ada QR code.
- Akses: super_admin (semua), agen/admin (cabangnya), kurir (hanya Shipment yang benar-benar ditugaskan padanya). Keuangan/korsal/sales/konsumen tidak mendapat akses cetak resi. Order yang `dibatalkan` tidak dapat dicetak resinya.
- Layout thermal-friendly (58mm default, toggle 80mm tersimpan per-browser di `localStorage` — bukan setting per-Agen, karena lebar kertas adalah properti printer fisik di satu komputer/loket, bukan konfigurasi bisnis cabang) di frontend route `/print/shipments/:id` (`ShipmentReceiptView.vue`), terpisah dari layout dashboard utama.
...
- Urutan kolom baku adalah: **Order No, Tanggal, SKU, Produk, Harga, Qty, Status Item, Subtotal, Konsumen, Tgl Kirim, Kurir, Status Order, Sales, Korsal, Metode Bayar, Grand Total, DP Dibayar, Total Dibayar, Sisa Pembayaran, Status Pembayaran, Status Additional Payment, Status Refund**. Mapping default Google Sheets mengikuti urutan ini dan dapat dipulihkan melalui tombol **Reset ke Default**.
...
- Kolom pembayaran (Metode Bayar s.d. Status Refund) bersumber dari `PaymentSummaryService` — formula kanonis tunggal yang juga dipakai `OrderResource.payment_summary` (Order Detail) dan dashboard Keuangan, supaya tidak ada tempat lain yang menghitung ulang dengan rumus berbeda. Nilai payment level-order ini **berulang identik** di setiap baris item milik order yang sama (sesuai desain "satu baris = satu order_item") — nilai tersebut boleh dipakai untuk konteks/detail per baris, tetapi **tidak boleh dijumlahkan** di laporan ringkasan finansial (itu akan menggandakan Grand Total order yang punya banyak item). Agregasi finansial selalu memakai level order/payment (`financeSummary`/`networkSummary`), bukan SUM dari laporan item-level ini. `Status Additional Payment`/`Status Refund` per baris hanya merefleksikan ledger milik item tersebut (via `OrderItem.additional_payment_id` dan `OrderItemAdjustment` miliknya sendiri) — item lain pada order yang sama tidak saling membocorkan status.
...
- OpenRouteService memakai origin koordinat profil Agen dan koordinat tujuan order. Jarak jalan, durasi, profile, rule tarif, koordinat, serta biaya final disimpan sebagai snapshot. Kegagalan provider menolak quote/order dengan error aman; sistem tidak mengganti hasil menjadi garis lurus, gratis, atau provider lain.
  Formula ongkir Kurir Online (kanonis, `OpenRouteProvider`): `chargeable_distance_km = max(0, route_distance_km − minimum_distance_km)`, lalu `shipping_cost = chargeable_distance_km × rate_per_km`. `minimum_distance_km` adalah jarak gratis yang dikurangkan dari jarak tempuh, bukan ambang gratis/berbayar — jarak sama dengan atau di bawah minimum selalu menghasilkan ongkir Rp0. `minimum_charge` (kalau diisi Agen) jadi batas bawah biaya setelah formula di atas dihitung.
...
- **Order Lifecycle Lengkap**: lihat §3.4 — termasuk penyesuaian fulfillment per item (yang sekarang menghitung ulang `Order.total_amount` secara otoritatif lewat `OrderTotalCalculator`, lihat §6 poin 3), reschedule tanggal kirim (termasuk split kuantitas), pembatalan, retur/refund per-item berbasis overpayment sungguhan, additional payment.
...
- **Pembayaran**: COD (dengan foto bukti + konfirmasi Keuangan), DP/Down Payment (nominal sebagian + pelunasan/settlement terpisah), Transfer Bank Manual (foto bukti + verifikasi Keuangan), dan 3 payment gateway (Xendit/Tripay/Stripe) dengan webhook terverifikasi signature serta idempotency guard. Order Detail menampilkan "Ringkasan Pembayaran" kanonis (`PaymentSummaryService`): Total Tagihan, DP Dibayar, Total Dibayar, Sisa Pembayaran, Status Pembayaran, Additional Payment, dan Refund — sumber yang sama dipakai laporan transaksi dan Google Sheets.
- **Pengiriman**: Kurir Online memakai OpenRouteService Directions V2 dengan origin koordinat Agen dan destination snapshot order (formula ongkir di §3.8); Ekspedisi memakai RajaOngkir/Komerce V2. Super Admin mengatur sakelar provider dan master ekspedisi global. Setiap Agen menyimpan kredensial terenkripsi, memilih kurir, dan mengatur tarifnya sendiri. Hasil provider dan tarif final dihitung server lalu disimpan sebagai snapshot shipment.
- **Print Resi Thermal**: lihat §3.4.1 — resi per-Shipment (bukan per-Order), mode Sebelum/Sesudah Pickup otomatis, 58mm/80mm, reprint aman (read-only, tercatat di audit log).
...
| Print Resi Thermal | `/print/shipments/:id` — resi sebelum/sesudah pickup per Shipment | Super Admin/Agen/Admin (cabangnya); Kurir hanya Shipment miliknya; Keuangan/Korsal/Sales/Konsumen tidak dapat akses |
...
3. **Refund/Additional Payment mengikuti uang sungguhan, bukan perubahan harga** — tiga nilai otoritatif: `current_order_total` (dihitung ulang oleh `OrderTotalCalculator` setiap item order berubah — **bukan** cuma sekali saat order dibuat), `total_valid_paid` (`Order.paid_amount`, satu-satunya kolom yang ditulis `PaymentService`), dan `balance = current_order_total − total_valid_paid`. Menambah/mengurangi item **tidak pernah otomatis** membuat Additional Payment/Refund hanya karena totalnya naik/turun — keduanya baru muncul kalau uang yang **sungguh sudah diterima** melewati/kurang dari tagihan baru:
   - `balance > 0` (masih ada tagihan) → itu `remaining_balance` biasa, bukan Refund maupun Additional Payment — berlaku untuk COD yang belum dibayar maupun DP yang masih partial.
   - `balance = 0` → LUNAS.
   - `total_valid_paid` melebihi total baru (overpaid) → **baru** itu Refund eligible, sebesar `overpaid_amount = max(0, total_valid_paid − current_order_total)` — bisa terjadi pada COD (tidak pernah, karena `paid_amount` COD yang belum dibayar = 0), DP yang sudah dibayar sebagian, maupun transaksi yang sudah LUNAS. Refund dihitung **inkremental** (selisih overpaid sebelum vs sesudah adjustment ini) supaya pengurangan berturut-turut tidak me-refund uang yang sama dua kali.
   - Order yang **sudah LUNAS sebelum** perubahan, lalu totalnya naik lagi (mis. tambah produk) dan melebihi `total_valid_paid` → **baru** itu Additional Payment, sebesar selisih tagihan baru dikurangi yang sudah dibayar. DP yang masih partial (belum lunas) tidak pernah memicu Additional Payment — penambahan tetap nempel ke `remaining_balance` order yang sama.
   - Menandai Additional Payment "paid" benar-benar menambah `Order.paid_amount` (`PaymentService::applyPaymentToOrder`); memproses Refund benar-benar mengurangi `Order.paid_amount` (`PaymentService::reverseAppliedPayment`) — keduanya idempoten (tidak bisa diproses dua kali) dan **tidak pernah** mengedit nominal histori payment yang sudah tercatat (§ poin 6 di bawah).
   - Pelunasan DP (settlement) selalu berupa `PaymentTransaction` bertipe `payment`, **bukan** `additional_payment` — meski dari sisi Keuangan terasa mirip "bayar lagi", secara ledger keduanya konsep berbeda dan tidak boleh tertukar.
